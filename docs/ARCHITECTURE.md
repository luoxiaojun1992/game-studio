...
- **Games**: directory-based artifact submission (using `games/latest/`), auto-generated `version_number` for unique identification, listing, file download via MinIO, and **Sonar report download**
...
- `file_storages`
  ...
  - `commands`
  - `permission_requests`
- Game artifacts are written to `output/{project_id}/games/latest/` by the engineer calling `write_game_file` tool, then packaged as ZIP by `submit_game`, uploaded to MinIO, and linked through `games.file_storage_id`
