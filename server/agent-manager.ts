...
`;
  ...
  private sendTeamBuilderSummaryRequest(projectId: string, sourceAgentId: AgentRole, sourceTask: string, failureReason?: string): void {
    ...
    // 读取配置的 Team Builder 模型，未配置时使用默认值
    let teamBuilderModel = 'glm-5.0';
    try {
      const settings = db.getProjectSettings(projectId);
      if (settings?.team_builder_model) {
        teamBuilderModel = settings.team_builder_model;
      }
    } catch { /* 降级：使用默认模型 */ }
  ...
  async sendMessage(
    ...
    try {
      ...
      const CAN_AUTO_ALLOW = [
        ...
        // submit_game 仅对 engineer 开放，且需要 UI 授权
        ...(isEngineer ? ['submit_game'] : []),
        // write_game_file 仅对 engineer 开放，但不需要 UI 授权（仅写入本地 output 目录）
        'write_game_file',
