...
class AgentManager extends EventEmitter {
    ...
    updateAgentState(agentId, updates) {
        const current = this.agentStates.get(agentId) || { id: agentId };
     ...
     * 添加日志
     */
    addLog(agentId, action, detail, level = 'info') {
        const log = {
            id: uuidv4(),
            agent_id: agentId,
            action,
     ...
     * 向 Agent 发送消息并获取流式响应
     */
    async sendMessage(agentId, message, model = 'glm-5.0', onEvent) {
        ...
        try {
            ...
            const startEvent = { type: 'agent_start', agentId, streamId, message };
            ...
            for await (const msg of stream) {
                // 检查是否被暂停中断
                if (this.isAgentPaused(agentId)) {
                    const pauseEvent = { type: 'agent_paused_mid_task', agentId, streamId };
                ...
                if (msg.type === 'system' && msg.subtype === 'init') {
                    ...
                    if (typeof content === 'string') {
                        ...
                        const textEvent = { type: 'text', agentId, content, streamId };
                        ...
                        for (const block of content) {
                            if (block.type === 'text') {
                                ...
                                const textEvent = { type: 'text', agentId, content: block.text, streamId };
                                this.emit('stream_event', textEvent);
                                if (onEvent)
                                    onEvent(textEvent);
                            }
                                ...
                                const toolEvent = { type: 'tool', agentId, id: toolId, name: block.name, input: toolInput, status: 'running', streamId };
                    ...
                    const doneEvent = { type: 'agent_done', agentId, streamId, duration: msg.duration, cost: msg.cost };
            ...
            const errorEvent = { type: 'agent_error', agentId, streamId, error: error?.message || String(error) };
