    ...
    tools: [
      ...
      tool(
        ...
        '将任务移交给其他团队成员。当你完成自己的工作部分，需要其他 Agent 接手时调用此工具。交接需要管理者确认后目标 Agent 才会开始工作。',
        {
          ...
          title: z.string().regex(NO_LINE_BREAKS_PATTERN, '标题不允许包含换行符').describe('简短的任务标题'),
      ...
      tool(
        ...
        '将一个功能目标拆分为开发任务和测试任务，并写入任务看板。',
        {
          ...
          feature_title: z.string().regex(NO_LINE_BREAKS_PATTERN, '标题不允许包含换行符').describe('功能标题'),
      ...
      tool(
        ...
        '提交一份策划案或方案文档（如游戏策划案、商业策划案、技术方案等）。提案提交后将通知管理者进行审批。',
        {
          ...
          title: z.string().regex(NO_LINE_BREAKS_PATTERN, '标题不允许包含换行符').describe('提案标题'),
