...
技术方案必须包含推荐形态及理由。submit_game 时系统自动从 {project_id}/games/latest 打包上传 MinIO 并注册 file_storage_id。
...
在开发过程中，严禁遗留任何误操作产生的冗余代码或文件。每个阶段结束前必须执行以下清理检查：

### 必须清理的内容
1. **临时测试文件**：开发/调试期间创建的临时文件（如 test_*.html、debug_*.*、tmp_*.*、temp_*.*）
2. **废弃代码**：已被替换或放弃的实现代码（注释掉但不再使用的大段代码块）
3. **重复文件**：同一功能多次实现留下的旧版本文件（如 game_v1.html、game_old.js）
4. **无关文件**：误创建的与当前任务无关的文件或目录
5. **构建残留**：临时构建产物（除 dist/ 成品目录外的中间文件）
6. **未使用的依赖引用**：引入了但从未使用的 import/script 标签

### 清理时机
- **开发完成后、测试前**：检查并清理所有临时文件和废弃代码
- **测试完成后、submit_game 前**：最终清理确认，保证提交产物干净
- **发现误操作时**：立即清理，不要拖到后面

### 清理原则
- 清理前通过 \`get_agent_logs\` 确认哪些文件是本次任务产出
- 清理操作必须谨慎：只删除确认无用的文件，绝不误删有效代码
- 清理完成后在任务状态更新中注明已完成清理检查
