...
export const jsSecurityChecker: LintChecker = {
  id: 'js-security',
  name: 'JS 安全检查',
  ...
  check(content: string, _context?: LintContext): LintIssue[] {
    ...
    issues.push(...findMatches(
      ...
      RE_EVAL,
      'js-eval',
      '检测到 eval() 调用。eval 存在代码注入风险，建议使用更安全的替代方案（如 JSON.parse）。',
      cid
    ));
    ...
    issues.push(...findMatches(
      ...
      '检测到 Function() 构造函数调用。动态代码执行存在安全风险，建议避免使用。',
      cid
    ));
    ...
    issues.push(...findMatches(
      ...
      RE_JS_URL,
      'js-js-url',
      '检测到 javascript: 协议 URL。可能存在 XSS 风险，建议使用事件监听替代。',
      cid
    ));
    ...
    issues.push(...findMatches(
      ...
      RE_INNER_HTML_WRITE,
      'js-inner-html-write',
      '检测到 innerHTML 赋值操作。若赋值内容来自用户输入，可能导致 XSS 攻击。建议使用 textContent 或 DOM API 替代。',
      cid
    ));
