...
class SonarQubeClient {
    ...
    async ensureProject(projectKey, projectName) {
        const getRes = await fetch(`${this.baseUrl}/api/projects/search?projects=${encodeURIComponent(projectKey)}`, { headers: this.authHeaders(), signal: AbortSignal.timeout(10000) });
        if (getRes.ok) {
            const data = await getRes.json();
            if (data.projects?.some(p => p.key === projectKey))
                return;
        }
    ...
    async submitAnalysis(projectKey, zipBuffer) {
        const form = new FormData();
        form.append('file', new Blob([zipBuffer], { type: 'application/zip' }), `${projectKey}.zip`);
        const res = await fetch(`${this.baseUrl}/api/sources/upload?project=${encodeURIComponent(projectKey)}`, { method: 'POST', headers: { Authorization: `Basic ${Buffer.from(`${this.token}:`).toString('base64')}` }, body: form, signal: AbortSignal.timeout(60000) });
        if (!res.ok) {
    ...
    async waitForTask(taskId, timeoutMs = 120000) {
        ...
        while (Date.now() < deadline) {
            ...
            const res = await fetch(`${this.baseUrl}/api/ce/task?id=${encodeURIComponent(taskId)}`, { headers: this.authHeaders(), signal: AbortSignal.timeout(15000) });
            if (!res.ok)
                continue;
            const data = await res.json();
    ...
    async getProjectIssues(projectKey) {
        ...
        let page = 1;
        while (true) {
            const res = await fetch(`${this.baseUrl}/api/issues/search?projects=${encodeURIComponent(projectKey)}&ps=100&p=${page}`, { headers: this.authHeaders(), signal: AbortSignal.timeout(15000) });
            if (!res.ok)
                throw new Error(`SonarQube issues 查询失败: ${res.status}`);
            const data = await res.json();
 ...
 */
function wrapHtmlForSonarQube(content, fileName) {
...
function resolveConfig(context) {
    const baseUrl = context?.sonarQubeUrl
        || `http://localhost:${process.env.SONARQUBE_PORT ?? '9002'}`;
    const token = context?.sonarQubeToken
