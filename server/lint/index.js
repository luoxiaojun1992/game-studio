 ...
 * @returns lint 检查结果
 */
export async function lintGameContent(htmlContent, context) {
    const runner = createLintRunner();
 ...
 * 构建人类可读的结果汇总文本
 */
function buildSummary(errors, warnings) {
    const lines = [];
    if (errors.length > 0) {
 ...
 * @returns lint 检查结果
 */
export async function lintZipBuffer(zipBuffer, context) {
    const { default: unzipper } = await import('unzipper');
    const zip = await unzipper.Open.buffer(zipBuffer);
    const htmlFiles = zip.files.filter((f) => !f.dir && /\.(html?|htm)$/i.test(f.path));
