 ...
 * @param logFn - 日志记录函数
 */
export function createStudioToolsServer(agentId, logFn) {
    const log = logFn || (() => { });
    const server = createSdkMcpServer({
        ...
        tools: [
            ...
            tool('create_handoff', '将任务移交给其他团队成员。当你完成自己的工作部分，需要其他 Agent 接手时调用此工具。交接需要管理者确认后目标 Agent 才会开始工作。', {
                to_agent_id: z.enum(['engineer', 'architect', 'game_designer', 'biz_designer', 'ceo']).describe('目标 Agent ID：engineer=软件工程师, architect=架构师, game_designer=游戏策划, biz_designer=商业策划, ceo=CEO'),
                ...
                return {
                    content: [{ type: 'text', text: `交接已创建 (ID: ${handoff.id.slice(0, 8)})，等待管理者确认后 ${to_agent_id} 才会开始工作。` }]
                };
            }),
            // ==================== 提案工具 ====================
 ...
 */
export function getMemorySummaryForPrompt(agentId) {
