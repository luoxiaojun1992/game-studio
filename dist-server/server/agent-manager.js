...
class AgentManager extends EventEmitter {
    constructor() {
        super();
        this.agentStatesByProject = new Map();
        this.pausedAgentsByProject = new Map();
        this.activeStreams = new Map();
        this.pendingPermissions = new Map();
        this.pendingPermissionExpirations = new Map();
        this.activeAgentStreamsByProject = new Map();
    ...
    ensureProjectState(projectId) {
        ...
        if (this.agentStatesByProject.has(scopedProjectId))
            return;
        const projectStates = new Map();
        const projectPausedSet = new Set();
        const projectActiveStreams = new Map();
        const agentIds = ['engineer', 'architect', 'game_designer', 'biz_designer', 'ceo'];
    ...
    getAllAgentStates(projectId) {
        const scopedProjectId = this.normalizeProjectId(projectId);
        this.ensureProjectState(scopedProjectId);
        return Array.from(this.agentStatesByProject.get(scopedProjectId).values());
    }
    updateAgentState(projectId, agentId, updates) {
        const scopedProjectId = this.normalizeProjectId(projectId);
        this.ensureProjectState(scopedProjectId);
        const projectStates = this.agentStatesByProject.get(scopedProjectId);
        const current = projectStates.get(agentId) || { id: agentId };
     ...
     */
    isAgentPaused(projectId, agentId) {
        const scopedProjectId = this.normalizeProjectId(projectId);
        this.ensureProjectState(scopedProjectId);
     ...
     */
    addLog(projectId, agentId, action, detail, level = 'info') {
        const scopedProjectId = this.normalizeProjectId(projectId);
        const entry = {
            id: uuidv4(),
            project_id: scopedProjectId,
     ...
     */
    addAgentLogEntry(projectId, agentId, logType, content, extra) {
        const scopedProjectId = this.normalizeProjectId(projectId);
        const level = logType === 'error' || extra?.isError ? 'error' : 'info';
        const entry = {
            id: uuidv4(),
            project_id: scopedProjectId,
            agent_id: agentId,
     ...
     */
    validateEngineerTaskBoardBeforeFinish(projectId) {
    ...
    buildHandoffTaskMessage(handoff) {
        return `【任务交接】你收到了来自 ${handoff.from_agent_id} 的任务交接。\n\n## 任务标题\n${handoff.title}\n\n## 任务描述\n${handoff.description}\n\n${handoff.context ? `## 上下文信息\n${handoff.context}\n\n` : ''}请按照上述要求完成任务。完成后请提交相关成果。`;
    }
    dispatchAutoHandoffTask(handoff) {
        this.addLog(handoff.project_id, handoff.to_agent_id, '自动接收交接', `从 ${handoff.from_agent_id} 接手: ${handoff.title}`, 'success');
        this.addLog(handoff.project_id, handoff.to_agent_id, '开始执行交接任务', `${handoff.title}`, 'success');
        this.sendMessage(handoff.project_id, handoff.to_agent_id, this.buildHandoffTaskMessage(handoff)).catch((error) => {
            this.addLog(handoff.project_id, handoff.to_agent_id, '交接任务执行失败', error?.message || String(error), 'error');
        });
    }
    /**
     * Builds the final system prompt by combining static role prompt and project memory summary.
     */
    buildSystemPrompt(projectId, agentId) {
     ...
     */
    async sendMessage(projectId, agentId, message, model = 'glm-5.0', onEvent) {
        const scopedProjectId = this.normalizeProjectId(projectId);
        this.ensureProjectState(scopedProjectId);
        ...
        try {
            ...
            const startEvent = { type: 'agent_start', projectId: scopedProjectId, agentId, streamId, message };
            this.emit('stream_event', startEvent);
            if (onEvent)
                onEvent(startEvent);
            for await (const msg of stream) {
                if (this.isAgentPaused(scopedProjectId, agentId)) {
                    const pauseEvent = { type: 'agent_paused_mid_task', projectId: scopedProjectId, agentId, streamId };
                ...
                if (msg.type === 'system' && msg.subtype === 'init') {
                    ...
                    if (typeof content === 'string') {
                        ...
                        const textEvent = { type: 'text', projectId: scopedProjectId, agentId, content, streamId };
                        ...
                        for (const block of content) {
                            if (block.type === 'text') {
                                ...
                                const textEvent = { type: 'text', projectId: scopedProjectId, agentId, content: block.text, streamId };
                                this.emit('stream_event', textEvent);
                                if (onEvent)
                                    onEvent(textEvent);
                            }
                                ...
                                const toolEvent = { type: 'tool', projectId: scopedProjectId, agentId, id: toolId, name: block.name, input: toolInput, status: 'running', streamId };
            ...
            const errorEvent = { type: 'agent_error', projectId: scopedProjectId, agentId, streamId, error: error?.message || String(error) };
     ...
     */
    respondToPermission(requestId, behavior, message, projectId, updatedInput) {
        const pending = this.pendingPermissions.get(requestId);
        if (!pending) {
     ...
     */
    getPendingPermissions(projectId) {
