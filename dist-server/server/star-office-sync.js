...
function buildProjectStatePayload(projectId, agents) {
    return {
...
class StarOfficeSyncService {
    constructor() {
        this.timerByProject = new Map();
        this.registeredAgents = new Map(); // key: `${projectId}:${agentRole}`
        this.supervisorTimer = null;
        this.wasOnline = true; // Track if Star-Office-UI was online last check
        this.currentProjectId = 'default';
        this.globalRegisterLock = false;
        this.pendingRegisterResolve = [];
    }
    isEnabled() {
        return !!joinAgentUrl;
    }
     ...
     */
    async getRemoteAgentIds() {
        if (!agentsUrl)
            return new Set();
        try {
            const agents = await this.getJson(agentsUrl);
     ...
     */
    async registerAgent(projectId, agentRole, agentName, state, detail) {
     ...
     */
    async ensureAgentRegisteredAndGetId(projectId, agentRole, state) {
     ...
     */
    notifyAgentStatusChanged(projectId, agentId, state) {
        if (!this.isEnabled())
            return;
     ...
     */
    async syncProjectState(projectId, reason) {
        if (!this.isEnabled())
            return;
        await this.acquireRegisterLock();
        try {
    ...
    scheduleProjectStateSync(projectId, reason) {
        if (!this.isEnabled())
            return;
     ...
     */
    async syncProjectStateIfCurrent(projectId, reason) {
        if (this.currentProjectId !== projectId) {
     ...
     */
    async switchProject(fromProjectId, toProjectId) {
        if (!this.isEnabled())
            return;
...
//# sourceMappingURL=star-office-sync.js.map