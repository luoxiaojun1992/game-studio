 ...
 */
import { z } from 'zod';
import { tool, createSdkMcpServer } from '@tencent-ai/agent-sdk';
 ...
 *
 */
export function createStudioToolsServer(projectId, agentId, logFn, onAutoHandoff) {
    const log = logFn || (() => { });
    const scopedProjectId = (projectId || 'default').trim() || 'default';
    const enforceProject = (requested) => {
    ...
    const server = createSdkMcpServer({
        ...
        version: '1.0.0',
        tools: [
            tool('save_memory', '保存一条长期记忆。在做出重要决策、获得经验教训、产出成果等关键时刻，你应该主动调用此工具保存信息。', {
                category: z.enum(['general', 'preference', 'decision', 'lesson', 'achievement']).describe('记忆分类：general=通用, preference=用户偏好, decision=重要决策, lesson=经验教训, achievement=成果产出'),
                ...
                return {
                    content: [{ type: 'text', text: `记忆已保存 (ID: ${memory.id.slice(0, 8)})` }]
                };
            }),
            tool('get_memories', '获取你之前保存的长期记忆，帮助你回忆之前的决策、经验和成果。', {
                ...
                if (memories.length === 0) {
                    return {
                        content: [{ type: 'text', text: '暂无保存的记忆。' }]
                    };
                }
                const text = memories.map(m => `[${m.category}/${m.importance}] (${m.created_at.slice(0, 10)}) ${m.content}`).join('\n');
                return {
                    content: [{ type: 'text', text }]
                };
            }),
            tool('create_handoff', '将任务移交给其他团队成员。当你完成自己的工作部分，需要其他 Agent 接手时调用此工具。交接需要管理者确认后目标 Agent 才会开始工作。', {
                to_agent_id: z.enum(['engineer', 'architect', 'game_designer', 'biz_designer', 'ceo']).describe('目标 Agent ID：engineer=软件工程师（含软件测试）, architect=架构师, game_designer=游戏策划（含UI设计）, biz_designer=商业策划, ceo=CEO'),
                title: z.string().describe('简短的任务标题'),
                description: z.string().describe('详细的任务描述'),
                context: z.string().optional().describe('上下文信息：你的工作成果摘要、相关文件路径、关键决策等。这些信息对下一个 Agent 完成任务至关重要。'),
                ...
                if (autoHandoffEnabled && onAutoHandoff) {
                    try {
                        ...
                        log(to_agent_id, '交接任务执行失败', `[${agentId} → ${to_agent_id}: ${title}] ${error?.message || String(error)}`, 'error');
                    }
                }
                return {
                    content: [{
            ...
            tool('split_dev_test_tasks', '将一个功能目标拆分为开发任务和测试任务，并写入任务看板。', {
            ...
            tool('get_tasks', '查询任务看板中的任务，用于查看待办和当前进度。', {
                ...
                if (tasks.length === 0) {
                    return { content: [{ type: 'text', text: '没有匹配的看板任务。' }] };
                }
            ...
            tool('update_task_status', '更新看板任务状态，维护开发与测试过程进度。', {
                ...
                if (!UUID_PATTERN.test(normalizedTaskId)) {
                    return { content: [{ type: 'text', text: `任务 ID 格式非法: ${normalizedTaskId}。${TASK_ID_HELP_TEXT}` }] };
                }
                const task = db.getTaskBoardTask(normalizedTaskId);
                if (!task) {
                    return { content: [{ type: 'text', text: `任务不存在: ${normalizedTaskId}。${TASK_ID_HELP_TEXT}` }] };
                }
                ...
                const now = new Date().toISOString();
                const updates = { status, updated_by: agentId };
                ...
                if (!success) {
                    return { content: [{ type: 'text', text: `任务状态更新失败: ${task_id}` }] };
                }
                ...
                return {
                    content: [{ type: 'text', text: `任务状态已更新: ${task.id} -> ${status}` }]
                };
            }),
            tool('submit_proposal', '提交一份策划案或方案文档（如游戏策划案、商业策划案、技术方案等）。提案提交后将通知管理者进行审批。', {
                project_id: z.string().optional().default('default').describe('项目 ID，用于归档到 /output/{project_id}/... 目录'),
                type: z.enum(['game_design', 'biz_design', 'tech_arch', 'tech_impl', 'ceo_review']).describe('提案类型：game_design=游戏策划, biz_design=商业策划, tech_arch=架构方案, tech_impl=技术方案'),
                ...
                return {
                    content: [{ type: 'text', text: `提案已提交 (ID: ${proposal.id.slice(0, 8)})，等待审批。` }]
                };
            }),
            tool('submit_game', '提交一个完成的游戏成品（单文件 HTML）。游戏将被保存到数据库和产出目录中。', {
                project_id: z.string().optional().default('default').describe('项目 ID，用于归档到 /output/{project_id}/... 目录'),
                ...
                sseBroadcaster.broadcast({ type: 'game_submitted', game: { ...game, html_content: undefined, hasContent: true }, filePath }, targetProjectId);
                log(agentId, '提交游戏', `游戏: ${name} v${version || '1.0.0'}${filePath ? ' → 已保存' : ''}`, 'success');
                return {
                    content: [{ type: 'text', text: `游戏已提交 (ID: ${game.id.slice(0, 8)})，名称: ${name}，版本: ${version || '1.0.0'}。` }]
                };
            }),
            tool('get_proposals', '查询已有的提案列表，用于了解当前项目的策划案进度。', {
                ...
                if (proposals.length === 0) {
                    return {
                        content: [{ type: 'text', text: '没有找到匹配的提案。' }]
                    };
                }
                const text = proposals.map(p => `[${p.status}] ${p.title} (作者: ${p.author_agent_id}, 类型: ${p.type}, ${p.created_at.slice(0, 10)})`).join('\n');
                return {
                    content: [{ type: 'text', text }]
                };
            }),
            tool('get_pending_handoffs', '查询待处理的任务交接，了解是否有其他 Agent 向你发起了交接。', {
                ...
                if (relevant.length === 0) {
                    return {
                        content: [{ type: 'text', text: '没有待处理的交接任务。' }]
                    };
                }
                const text = relevant.map(h => `[${h.status}] ${h.title} (来自: ${h.from_agent_id}, 优先级: ${h.priority}, ${h.created_at.slice(0, 10)})\n  描述: ${h.description.slice(0, 100)}`).join('\n\n');
                return {
                    content: [{ type: 'text', text }]
                };
            })
...
/**
 */
export function getMemorySummaryForPrompt(projectId, agentId) {
    const memories = db.getAgentMemories(projectId, agentId, undefined, 20);
    if (memories.length === 0)
        return '';
    const summary = memories.map(m => `- [${m.category}/${m.importance}] ${m.content}`).join('\n');
    return `
