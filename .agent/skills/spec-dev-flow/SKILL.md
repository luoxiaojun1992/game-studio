...
description: >
  ...
  确认编号（读取 .agent/specs/INDEX.md 列出未实现项）→ 阅读 memory 文档 → 阅读 spec → 规划任务 → 实现 → review → code-lint → 推送分支 →
...
1. **读取 SPEC 索引**：读取 `.agent/specs/INDEX.md`，解析索引表
2. **筛选未实现项**：提取所有状态为 `设计中` 的 SPEC，按编号从小到大排序
3. **展示可选列表**：将未实现的 SPEC 以编号表格形式呈现给用户选择：
...
| SPEC-013 | 游戏测试微服务规范 |
| SPEC-018 | 分层架构重构规范 |
| SPEC-020 | OpenTelemetry 分布式链路追踪 |
| SPEC-021 | Team Building Agent 指示灯 |
```

4. **用户选择**：
   - 用户可能输入完整编号（`SPEC-011`）、纯数字（`11`、`011`）
   - **自动补全**：纯数字自动补全为 `SPEC-XXX` 格式（不足三位前面补零）
   - **校验**：确认所选编号在未实现列表中，如果不是则提示用户重新选择
   - 确认后告知用户将要开发的 SPEC 全称和文件路径
