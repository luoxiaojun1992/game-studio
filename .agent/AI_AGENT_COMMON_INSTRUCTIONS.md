...
**1. 先思考再动手**
- 不假想、不掩盖困惑——把假设和疑虑摆在明面上
- 存在多种解释时，先呈现再选择，不要静默pick
- 有更简单的方案时，直接说出来

**2. 简洁优先**
- 只写解决问题所必需的代码，不写"以防万一"的代码
- 不为单次使用的代码创建抽象
- 不引入未要求的功能或可配置项
- 自问："一个 senior engineer 会觉得这是过度设计吗？"如果答案是是，就简化

**3. 精准改动**
- 只改必须改的，不"顺便"优化无关代码、注释或格式
- 与现有风格保持一致，即使你自己有不同偏好
- 变更产生的孤儿代码（未使用的 import/变量/函数）要清理
- 每次改动的每一行都应能追溯到用户请求

**4. 目标驱动执行**
- 把任务转化为可验证的目标："加验证" → "为无效输入写测试，然后让它们通过"
- 多步骤任务先列出简短计划，说明每个步骤的验证方式
- 强成功标准让各步骤可独立循环；弱标准（"让它能用"）则需持续澄清
