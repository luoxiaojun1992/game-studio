...
- 注意：`lastMessage` 属于内存态，当前不落库；持久化主要覆盖 `status/current_task/sdk_session_id/updated_at`。
- 含义：服务重启后 `lastMessage` 会丢失，但不影响可恢复状态（status/current_task/paused）和历史消息查询（`agent_messages`）。
