...
| SonarQube health check 路径用 `/api/system/health`（仅 UP 时存在）导致 STARTING 阶段总是失败 | 应改为 `/api/system/status`（STARTING 时就返回），并验证 `status` 字段值为 `UP` | 服务在 STARTING 阶段被标记 unhealthy，但实际已就绪 |
